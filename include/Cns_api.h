...
EXTERN_C int Cns_download_seg(const char *path, off_t offset, size_t size, char *location, size_t filesize, char *buff);
...
EXTERN_C int xrd_read(const char *path, size_t size, off_t offset, char *buff, char *path_t, size_t filesize);
...
EXTERN_C int xrd_refresh_metadata(char *path);
